    ...
    public partial class Form1 : Form
    {
        ...
        private void box_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            if (e.Button == MouseButtons.Left)
            {
                ...
                //try
                //{
                //    if (int.Parse(x[1, 1].Text) == 0 || int.Parse(x[0, 1].Text) == 0 || int.Parse(x[1, 0].Text) == 0)
                //    {
                //        x[0, 0].Text = "0";
                //        x[0, 0].Font = new Font("方正清刻本悦宋", 20);
                //        x[0, 0].ForeColor = Color.Blue;
                //    }
                ...
                //try
                //{
                //    for (int s = 0; s <= 9; s++) { if (x[0, s].Text != "") { if (int.Parse(x[0, s].Text) == 0) x[0, s + 1].Text = num[0 + 1, s + 1 + 1].ToString(); } };//外围也连成一片
                //    for (int s = 0; s <= 9; s++) if (x[s, 10].Text!=""){if (int.Parse(x[s, 10].Text) == 0) x[s + 1, 10].Text = num[s + 1 + 1, 10 + 1].ToString();};
                //    for (int s = 9; s >= 1; s--) if (x[10, s].Text!=""){if (int.Parse(x[10, s].Text) == 0) x[10, s + 1].Text = num[10 + 1, s + 1 + 1].ToString();};
                //    for (int s = 9; s >= 1; s--) if (x[s, 0].Text!=""){if (int.Parse(x[s, 0].Text) == 0) x[s + 1, 0].Text = num[s + 1 + 1, 0 + 1].ToString();};

                //    for (int s = 0; s <= 9; s++) if (x[s, 0].Text!=""){if (int.Parse(x[s, 0].Text) == 0) x[s + 1, 0].Text = num[s + 1 + 1, 0 + 1].ToString();};
                //    for (int s = 0; s <= 9; s++) if (x[10, s].Text!=""){if (int.Parse(x[10, s].Text) == 0) x[10, s + 1].Text = num[10 + 1, s + 1 + 1].ToString();};
                //    for (int s = 9; s >= 1; s--) if (x[s, 10].Text!=""){if (int.Parse(x[s, 10].Text) == 0) x[s + 1, 10].Text = num[s + 1 + 1, 10 + 1].ToString();};
                //    for (int s = 9; s >= 1; s--) if (x[0, s].Text != "") { if (int.Parse(x[0, s].Text) == 0) x[0, s + 1].Text = num[0 + 1, s + 1 + 1].ToString(); };
                //}
                //catch { }
            ...
            if (y == 0)
            {
                for (int i = 0; i < n; i++)
                {
                    for (int j = 0; j < n; j++)
                    {
                        x[i, j] = new Label();
                        x[i, j].Size = new Size(30, 30);
                        x[i, j].Location = new Point(20 + 32 * i, 20 + 32 * j);//把label放在合适的位置上
                        x[i, j].AutoSize = false;
                        x[i, j].BorderStyle = BorderStyle.Fixed3D;
                        this.Controls.Add(x[i, j]);
                        x[i, j].Click += new EventHandler(this.lab_Click);
